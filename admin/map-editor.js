...
// admin/ is an internal calibration tool with a single user and stays
// Romanian-only, outside the scope of the bilingual interface
// (docs/I18N_ARHITECTURA.md §3.4). It still goes through t() so its strings live
// in one place; it simply pins the language rather than offering a switch.
import { t, init as initI18n } from '../js/i18n/index.js';
...
class MapEditor {
  ...
  async init() {
    await initI18n({ basePath: '../', lang: 'ro' });
    this.checkServer();
  ...
  async checkServer() { try { const response = await fetch('../api/ping'); const payload = await response.json(); if (!response.ok || payload?.ok !== true) throw Error('invalid ping'); } catch { this.serverWarning = t('editor.serverWarning'); this.renderServerWarning(); } }
  ...
  renderSidebar() {
    if (this.view !== 'map') {
      document.querySelector('#editor-sidebar').innerHTML = `<h1>${t('editor.adminTitle')}</h1>${this.navMarkup()}<p class="admin-side-note">${t('editor.adminSideNote')}</p>`;
      return this.bindNav();
    }
    ...
    document.querySelector('#editor-sidebar').innerHTML = `${this.serverWarning ? `<p id="server-warning" class="editor-server-warning">${this.serverWarning}</p>` : ''}<h1>${t('editor.editorTitle')}</h1><p class="editor-subtitle">${t('editor.editorSubtitle')}</p><p class="editor-drag-help">${t('editor.dragHelp')}</p><div class="editor-progress">${t('editor.progress', { calibrated, total: locations.length })}</div><div class="editor-controls"><input data-filter placeholder="${t('editor.filterPlaceholder')}" value="${this.escape(this.filter)}"><select data-type><option value="all">${t('editor.types.all')}</option><option value="castle">${t('editor.types.castle')}</option><option value="city">${t('editor.types.city')}</option><option value="town">${t('editor.types.town')}</option><option value="fortress">${t('editor.types.fortress')}</option><option value="ruins">${t('editor.types.ruins')}</option><option value="landmark">${t('editor.types.landmark')}</option></select><button class="editor-next" data-action="next">${t('editor.btnNext')}</button></div><div class="editor-selection">${this.selectedMarkup()}</div><div class="editor-actions"><button data-action="save">${t('editor.btnSave')}</button><button data-action="undo">${t('editor.btnUndo')}</button><button class="danger" data-action="delete">${t('editor.btnDelete')}</button><button class="danger" data-action="all">${t('editor.btnDeleteAll')}</button><button data-action="validate">${t('editor.btnValidate')}</button><button data-action="copy">${t('editor.btnCopyJson')}</button><button data-action="export">${t('editor.btnExportJson')}</button><label><button type="button" data-action="import">${t('editor.btnImportJson')}</button><input hidden type="file" data-import accept="application/json"></label></div><div class="editor-list">${[...grouped].map(([key, items]) => `<details class="editor-group" open><summary>${key} (${items.length})</summary>${items.map(item).join('')}</details>`).join('') || `<p class="editor-empty">${t('editor.emptyUnset')}</p>`}<details class="editor-group editor-placed"${placedOpen ? ' open' : ''}><summary>${t('editor.placedSummary', { count: placed.length })}</summary>${placed.map(item).join('') || `<p class="editor-empty">${t('editor.emptyPlaced')}</p>`}</details></div>`;
    document.querySelector('#editor-sidebar h1').insertAdjacentHTML('afterend', this.navMarkup()); document.querySelector('[data-action="all"]')?.insertAdjacentHTML('afterend', `<button class="danger" data-action="retract-castles">${t('editor.btnRetractCastles')}</button><button data-action="distribute-all">${t('editor.btnDistributeAll')}</button>`); this.bindNav();
  ...
  renderSidebarList() {
    ...
    list.innerHTML = `${[...grouped].map(([key, items]) => `<details class="editor-group" open><summary>${key} (${items.length})</summary>${items.map(item).join('')}</details>`).join('') || `<p class="editor-empty">${t('editor.emptyUnset')}</p>`}<details class="editor-group editor-placed"${placedOpen ? ' open' : ''}><summary>${t('editor.placedSummary', { count: placed.length })}</summary>${placed.map(item).join('') || `<p class="editor-empty">${t('editor.emptyPlaced')}</p>`}</details>`;
  ...
  selectedMarkup() { const loc = this.locations.find(l => l.id === this.selectedId), p = loc && this.coordinate(loc.id); return loc ? `<strong>${this.escape(loc.name)}</strong><span class="editor-coordinates">${p ? `x: ${p.x}, y: ${p.y}` : t('editor.coordinatesUnset')}</span>` : t('editor.selectLocationHint'); }
  ...
  setPlacementHint(location, reposition = false) { const hint = document.querySelector('#editor-placement-hint'); if (!hint) return; hint.hidden = !location; hint.textContent = location ? t('editor.placementHint', { action: reposition ? t('editor.actionReposition') : t('editor.actionPlace'), name: location.name }) : ''; }
  ...
  finishDrag(e) { const d = this.drag; if (!d || (e?.pointerId != null && e.pointerId !== d.pointerId)) return; const target = e && this.dropTarget(e.clientX, e.clientY); this.removeGhost(); d.sourceElement?.classList.remove('dragging'); this.drag = null; this.ignoreMapClickUntil = d.source === 'map' ? Date.now() + 120 : 0;
    ...
    if (target === 'map') { const p = screenToMapPoint(this.renderer.svg, e.clientX, e.clientY); if (p.inside) { this.remember(d.id); this.setCoordinate(d.id, p, false); this.toast(t('editor.toastUpdated', { name: this.locationName(d.id) })); return; } }
    if (target === 'sidebar' && d.source === 'map') { this.remember(d.id); this.overrides[d.id] = null; window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); requestAnimationFrame(() => this.decorateMarkers()); this.renderSidebar(); this.toast(t('editor.toastDeleted', { name: this.locationName(d.id) })); return; }
  ...
  placeCarry(p) {
    ...
    this.toast(t('editor.toastUpdated', { name: this.locationName(id) }));
  }
  cancelCarry() {
  ...
  retractCarryToSidebar() {
    ...
    requestAnimationFrame(() => this.decorateMarkers());
    this.renderSidebar();
    this.toast(t('editor.toastDeleted', { name: this.locationName(id) }));
  }
  clearCarryState() {
  ...
  undo() { const item = this.undoStack.pop(); if (!item) return this.toast(t('editor.toastNoUndo')); this.overrides[item.id] = item.previous; window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); requestAnimationFrame(() => this.decorateMarkers()); this.select(item.id, false); this.toast(t('editor.toastUndoDone')); }
  async action(a) { if (a === 'next') { const l = this.locations.find(x => !this.hasCoordinate(x.id)); if (l) this.select(l.id); else this.toast(t('editor.toastAllHaveCoords')); } if (a === 'save') await this.save(); if (a === 'undo') this.undo(); if (a === 'delete') this.removeSelected(); if (a === 'all') this.removeAll(); if (a === 'retract-castles') this.retractCastles(); if (a === 'distribute-all') this.distributeAll(); if (a === 'validate') this.validate(); if (a === 'copy') navigator.clipboard.writeText(JSON.stringify(this.payload(), null, 2)).then(() => this.toast(t('editor.toastCopied'))); if (a === 'export') this.download(this.payload(), 'world_coordinates.json'); if (a === 'import') document.querySelector('[data-import]').click(); }
  distributeAll() { const missing = this.locations.filter(l => !this.hasCoordinate(l.id)); if (!missing.length) return this.toast(t('editor.toastDistributeNoMissing')); if (!confirm(t('editor.confirmDistribute', { count: missing.length }))) return; const byRegion = new Map(); missing.forEach(l => byRegion.set(l.region, [...(byRegion.get(l.region) || []), l])); byRegion.forEach((items, region) => { const polygon = pathToPolygon(this.regions.get(region)), candidates = this.gridPoints(polygon, getPolygonBounds(polygon), items.length); items.forEach((l, index) => { this.remember(l.id); this.overrides[l.id] = this.anchorFor(l) || candidates[index] || this.regionCenter(region) || { x: 750, y: 500 }; }); }); window.atlasWorldCoordinateOverrides=this.overrides; this.renderer.refreshLocationMarkers(); requestAnimationFrame(()=>this.decorateMarkers()); this.renderSidebar(); this.toast(t('editor.toastDistributed', { count: missing.length })); }
  ...
  removeSelected() { if (!this.selectedId || !confirm(t('editor.confirmDeleteOne', { name: this.locationName(this.selectedId) }))) return; this.remember(this.selectedId); this.overrides[this.selectedId] = null; window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); this.renderSidebar(); this.toast(t('editor.toastDeletedSave')); }
  removeAll() { if (!confirm(t('editor.confirmDeleteAll1')) || !confirm(t('editor.confirmDeleteAll2'))) return; this.locations.forEach(l => { this.remember(l.id); this.overrides[l.id] = null; }); window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); this.renderSidebar(); this.toast(t('editor.toastAllPreparedDelete')); }
  retractCastles() { const castles = this.locations.filter(location => location.type === 'castle' && this.hasCoordinate(location.id)); if (!castles.length) return this.toast(t('editor.toastNoCastles')); if (!confirm(t('editor.confirmRetractCastles1', { count: castles.length })) || !confirm(t('editor.confirmRetractCastles2'))) return; castles.forEach(location => { this.remember(location.id); this.overrides[location.id] = null; }); window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); requestAnimationFrame(() => this.decorateMarkers()); this.renderSidebar(); this.toast(t('editor.toastCastlesRetracted', { count: castles.length })); }
  ...
  async save() { if (!Object.keys(this.overrides).length) return this.toast(t('editor.toastNoChangesToSave')); try { const res = await fetch(API, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(this.payload()) }); let body; try { body = await res.json(); } catch { throw new Error(t('editor.errorInvalidJsonHttp', { status: res.status })); } if (!res.ok) throw new Error(body.error || t('editor.errorSaveFailedHttp', { status: res.status })); const unknown = new Set(body.unknownIds || []); const unsaved = {}; Object.entries(this.overrides).forEach(([id, point]) => { if (unknown.has(id)) unsaved[id] = point; else if (point) this.dataManager.data.worldCoordinates[id] = point; else delete this.dataManager.data.worldCoordinates[id]; }); this.overrides = unsaved; window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); this.renderSidebar(); if (body.updated) this.toast(t('editor.toastSavedUpdated', { count: body.updated })); const warning = unknown.size ? t('editor.warningUnsavedIds', { ids: [...unknown].join(', ') }) : ''; this.setSaveWarning(warning); if (warning) this.toast(warning, true); } catch (err) { this.toast(t('editor.errorCannotSave', { message: err.message }), true); } }
  validate() { const warnings = []; this.locations.forEach(l => { const p = this.coordinate(l.id), path = this.regions.get(l.region); if (p && !ISLANDS.has(l.id) && path && !isPointInPolygon(p, path)) warnings.push(l.name); }); this.toast(warnings.length ? t('editor.warningOutRegion', { names: warnings.join(', ') }) : t('editor.validationClean'), warnings.length > 0); }
  importFile(file) { if (!file) return; const reader = new FileReader(); reader.onload = () => { try { Object.entries(JSON.parse(reader.result).coordinates || JSON.parse(reader.result)).forEach(([id, p]) => { if (this.locations.some(l => l.id === id)) this.overrides[id] = p; }); window.atlasWorldCoordinateOverrides = this.overrides; this.renderer.refreshLocationMarkers(); this.renderSidebar(); this.toast(t('editor.toastImportSuccess')); } catch { this.toast(t('editor.toastImportInvalid'), true); } }; reader.readAsText(file); }
  ...
  navMarkup() { return `<div class="editor-nav"><button data-view="map" class="${this.view==='map'?'active':''}">${t('editor.navMap')}</button><button data-view="locations" class="${this.view==='locations'?'active':''}">${t('editor.navLocations')}</button><button data-view="houses" class="${this.view==='houses'?'active':''}">${t('editor.navHouses')}</button></div>`; }
  ...
  renderAdmin() { const houses=this.view==='houses', list=(houses?this.dataManager.data.houses:this.locations).filter(x=>JSON.stringify(x).toLowerCase().includes(this.adminFilter.toLowerCase())); const p=document.querySelector('#editor-admin-panel'); p.innerHTML=`<div class="admin-toolbar"><h2>${houses?t('editor.navHouses'):t('editor.navLocations')}</h2><input data-search placeholder="${t('editor.adminSearchPlaceholder')}" value="${this.escape(this.adminFilter)}"><button data-create>${t('editor.adminAdd')}</button></div><table class="admin-table"><thead><tr><th>Nume</th><th>${houses?t('editor.adminSeat'):t('editor.adminType')}</th><th>${houses?t('editor.adminWords'):t('editor.adminContinent')}</th><th>${houses?t('editor.adminFounded'):t('editor.adminRegion')}</th><th>${t('editor.adminActions')}</th></tr></thead><tbody>${list.sort((a,b)=>a.name.localeCompare(b.name)).map(x=>`<tr><td>${houses?`<img class="house-crest-preview" src="../${x.crest}" alt=""> `:''}${this.escape(x.name)}</td><td>${houses?x.metadata?.seat||'':x.type}</td><td>${houses?this.escape(x.metadata?.words||''):x.continent||''}</td><td>${houses?this.escape(x.metadata?.founded||''):x.region||''}</td><td><button data-edit="${x.id}">${t('editor.adminEdit')}</button><button data-delete="${x.id}">${t('editor.adminDelete')}</button></td></tr>`).join('')}</tbody></table>`;p.querySelector('[data-search]').oninput=e=>{this.adminFilter=e.target.value;this.renderAdminRows(houses);};p.querySelector('[data-create]').onclick=()=>this.entityForm(houses);p.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>this.entityForm(houses,b.dataset.edit));p.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>this.deleteEntity(houses,b.dataset.delete)); }
...
window.addEventListener('DOMContentLoaded', () => new MapEditor().init().catch(err => { console.error(err); document.body.innerHTML = `<pre>${t('editor.errorFailedStart', { message: err.message })}</pre>`; }));
